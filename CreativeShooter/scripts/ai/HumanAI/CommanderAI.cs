...
public class CommanderAI : MonoBehaviour
{
    ...
    private void AttackOrDefense()
    {
        ...
        if (hasOwn)
            AttackWeight += (_OwnBehavior.CurrentHealth / _OwnBehavior.BehaviorParameters.MaxHealth) * 25 + 60;
        if (hasArtilleryman)
        ...
        if (hasCharge)
            AttackWeight += (_ChargeBehavior.CurrentHealth / _ChargeBehavior.BehaviorParameters.MaxHealth) * 30 + 60;
        if (hasTank)
        ...
        //有以下角色,分別算出各自的數值後用防禦權重去加,血量*基礎數值 + 補數(我方為60 敵方為50)
        if (!hasOwn)
            DefenseWeight += (_OwnBehavior.CurrentHealth / _OwnBehavior.BehaviorParameters.MaxHealth) * 25 + 60;
        ...
        if (!hasCharge && _Charge != null)
            DefenseWeight += (_ChargeBehavior.CurrentHealth / _ChargeBehavior.BehaviorParameters.MaxHealth) * 30 + 60;
