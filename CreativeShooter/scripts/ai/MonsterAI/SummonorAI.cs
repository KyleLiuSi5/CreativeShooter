...
public class SummonorAI : MonoBehaviour {
    ...
    //可獲取Monster方角色狀態
    public CharacterBehavior _TankerBehavior;
    ...
    private Unit _Astar;
    //Summoner的技能
    private SummonExplosionMonster _SummonEX;//召喚爆炸怪
    private SummonZombie _SummonZ;//召喚殭屍
    private SummonSmallMonster _SummonS;//召喚小怪
    ...
    void Start()
    {
        ...
        if (GameObject.Find("Tanker") != null)
        {
            ...
            _TankerBehavior = _Tanker.GetComponent<CharacterBehavior>();
        ...
        _SummonEX = GetComponent<SummonExplosionMonster>();
        _SummonZ = GetComponent<SummonZombie>();
        _SummonS = GetComponent<SummonSmallMonster>();
    ...
    private void GetStatus()
    {
        ...
        if (_Tanker != null)
        {
            if (_Tanker.transform.position.x < (gameObject.transform.position.x + TeamDistance) && _Tanker.transform.position.x > (gameObject.transform.position.x - TeamDistance)
            && _Tanker.transform.position.y < (gameObject.transform.position.y + TeamDistance) && _Tanker.transform.position.y > (gameObject.transform.position.y - TeamDistance) && _TankerBehavior.CurrentHealth >= _TankerBehavior.BehaviorParameters.MaxHealth * 0.1)
            {
                TeamCount++;
                hasTanker = true;
                _TankerBlood = _Tanker.GetComponent<CharacterBehavior>().CurrentHealth;
    ...
    private void UseSkill()
    {
        ...
        {
            if (_SummonEX.CanPushT == true)
            {
                _SummonEX.PushT = true;
                _SummonEX.PushTCDTime = 0;
                _SummonEX.CanPushT = false;
                RunAwayTime = 1;
            }
            else if (_SummonEX.CanPushT == false && _SummonZ.CanPushR == true)
            {
                gameObject.transform.position = TeamCastlePosition.position;
                _SummonZ.CanPushR = false;
                _SummonZ.PushRCDTime = 0;
                RunAwayTime = 1;
            }
            else if (_SummonZ.CanPushR == false && _SummonEX.CanPushT == false && _SummonS.CanPushE == true)
            {
                _SummonS.PushE = true;
                _SummonS.CanPushE = false;
                _SummonS.PushECDTime = 0;
                RunAwayTime = 1;
            }
            else if (_SummonEX.CanPushT == false && _SummonZ.CanPushR == false && _SummonS.CanPushE == false)
                return;
        }
    ...
    private void AttackOrDefense()
    {
        ...
        if (hasTanker)
            AttackWeight += (_TankerBehavior.CurrentHealth / _TankerBehavior.BehaviorParameters.MaxHealth) * 30 + 60;
        if (hasShooter)
        ...
        //有以下角色,分別算出各自的數值後用防禦權重去加,血量*基礎數值 + 補數(我方為60 敵方為50)
        if (!hasTanker && _Tanker != null)
            DefenseWeight += (_TankerBehavior.CurrentHealth / _TankerBehavior.BehaviorParameters.MaxHealth) * 30 + 60;
