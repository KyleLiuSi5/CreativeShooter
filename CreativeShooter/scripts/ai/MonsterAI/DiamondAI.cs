...
public class DiamondAI : MonoBehaviour
{
    ...
    //可獲取Monster方角色狀態
    public CharacterBehavior _TankBehavior;
    ...
    //Diamond的技能
    private Avatar _avatar;//分身
    private Teleport _teleport;//瞬間移動
    ...
    void Start()
    {
        ...
        if (GameObject.Find("Tanker") != null)
        {
            ...
            _TankBehavior = _Tank.GetComponent<CharacterBehavior>();
    ...
    private void GetStatus()
    {
        ...
        if (_Tank != null)
        {
            if (_Tank.transform.position.x < (gameObject.transform.position.x + TeamDistance) && _Tank.transform.position.x > (gameObject.transform.position.x - TeamDistance)
            && _Tank.transform.position.y < (gameObject.transform.position.y + TeamDistance) && _Tank.transform.position.y > (gameObject.transform.position.y - TeamDistance) && _TankBehavior.CurrentHealth >= _TankBehavior.BehaviorParameters.MaxHealth * 0.1)
            {
                TeamCount++;
                hasTank = true;
                _TankBlood = _Tank.GetComponent<CharacterBehavior>().CurrentHealth;
    ...
    private void UseSkill()
    {
        if (CurrentHealth < _OwnBehavior.BehaviorParameters.MaxHealth * 0.2 && EnemyCount >= 1)
        {
            if (_avatar.CanPushT == true)
            {
                ...
                _avatar.CanPushT = false;
                RunAwayTime = 1;
                if (_avatar.PushT == true)
                    _avatar.PushT = false;
            }
            else if (_avatar.CanPushT == false && _teleport.CanPushE == true)
            {
                gameObject.transform.position =_Target.position;
                _teleport.CanPushE = false;
                _teleport.PushE = false;
                _teleport.TeleportCDTime = 0;
                RunAwayTime = 1;
            }
            else if (_avatar.CanPushT == false && _teleport.CanPushE == false && _Stealth.CanPushR == true)
            {
                _Stealth.PushR = true;
                _Stealth.CanPushR = false;
                _Stealth.StealthCDTime = 0;
                RunAwayTime = 1;
                
            }
            else if (_avatar.CanPushT == false && _teleport.CanPushE == false && _Stealth.CanPushR == false)
                return;
        }
    ...
    private void AttackOrDefense()
    {
        ...
        if (hasTank)
            AttackWeight += (_TankBehavior.CurrentHealth / _TankBehavior.BehaviorParameters.MaxHealth) * 30 + 60;
        if (hasShooter)
            AttackWeight += (_ShooterBehavior.CurrentHealth / _ShooterBehavior.BehaviorParameters.MaxHealth) * 30 + 60;
        if (hasSummoner)
            AttackWeight += (_SummonerBehavior.CurrentHealth / _SummonerBehavior.BehaviorParameters.MaxHealth) * 25 + 60;
        if (hasOwn)
        ...
        //有以下角色,分別算出各自的數值後用防禦權重去加,血量*基礎數值 + 補數(我方為60 敵方為50)
        if (!hasTank && _Tank != null)
            DefenseWeight += (_TankBehavior.CurrentHealth / _TankBehavior.BehaviorParameters.MaxHealth) * 30 + 60;
        ...
        if (!hasSummoner && _Summoner != null)
            DefenseWeight += (_SummonerBehavior.CurrentHealth / _SummonerBehavior.BehaviorParameters.MaxHealth) * 25 + 60;
        if (!hasOwn)
